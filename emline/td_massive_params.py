...
def load_obs(photname, objname, err_floor=0.05, zperr=True, **extras):
    ...
    obs['width_effective'] = np.array([filt.effective_width for filt in obs['filters']])
...
class FracSFH(FastStepBasis):
    ...
    def add_emission_lines(self,wave,spec,z):
        """adds emission lines to spectrum as delta functions
        """
        emlams = self.emline_wavelengths * (1+z)
        elums = self.get_nebline_luminosity # Lsun / solar mass formed
        for lam, lum in zip(emlams,elums):
            idx = (np.abs(wave-lam)).argmin()
            spec[idx] += 2*lum/(wave[idx+1]-wave[idx-1]) # convert to Lsun / solar mass formed / Angstrom in observed frame
        return spec
    ...
    def get_spectrum(self, outwave=None, filters=None, peraa=False, **params):
        ...
        if (not self.params['nebemlineinspec']) and self.params['add_neb_emission']:
            #spec_aa = self.add_emission_lines(wa,spec_aa,a-1)
